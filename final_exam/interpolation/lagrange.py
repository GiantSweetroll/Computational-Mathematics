...
#Solve lagrangian interpolation

def interpolate_lagrangian(xSymb, xy_values, n, xVal):
    """
    Interpolate using Lagrangian Interpolation.
    
    Params:
    xSymb - A Sympy Symbol object
    xy_values - a list of your known data points
    n - order of polynomial to use
    xVal - Value of x to interpolate
    
    returns the equation of the line and the result of the interpolation of your xVal
    """
    
    #Check for n
    if (n > len(xy_values)-1):
        n = len(xy_values) - 1  #Set to use highest order of polynomial
    elif (n < 1):
        n = 1
    ...
    get_remaining_indexes(xy_values, indexes, xVal, n)
    wFunc = gather_weighting_functions(n)
    equation = get_equation(xy_values, wFunc, indexes, xSymb)
    result = equation.evalf(subs={xSymb : xVal})
